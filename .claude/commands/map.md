...
Use Playwright to open each map style in a headless browser, wait for the map to render, then extract the SVG and save it to the `maps/$ARGUMENTS/` directory. The filename should be `{campaign}-{style}.svg`.
